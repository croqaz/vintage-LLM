...
We want to steam-tune the LLM until it behaves like a Victorian person from the year 1899.
All the datasets we use have a hard cut at year 1900.

**Target year: 1899.** This matches the data — every system prompt in `vintage-ft-v1` says
...
**What counts as an anachronism is already written down — do not reinvent it.**
[`knowledge/README.md`](../../knowledge/README.md) holds the curated lists:
`## Allowed knowledge (before year 1900)` and `## Banned knowledge (after the year 1900)`,
with dates and reasoning per item (electrical telegraph 1840s, soda siphon 1829, …; World
Wars, Great Depression, Cold War, viruses-as-particles 1931, … all banned). The machine-
readable side is `knowledge/pre1900/banned_terms.py` (~800 terms). Any prompt set, filter or
eval in this project must derive its allowed/banned notion from **those two files**, never
from a list improvised in a config or a docstring.

## The question Experiment 1 answers

> Does `training/fine_tune.py` + a per-experiment TOML + `vintage-ft-v1` produce a model that
> writes like 1899 — **and does LoRA even work in this script?**

Not "can we produce good models" in the abstract. One question, two arms, an attributable answer.

## 📋 Status — see [`EXPERIMENT_LOG.md`](EXPERIMENT_LOG.md) for the full record
...
```
AMD Radeon RX 9070 XT  ·  17.1 GB VRAM  ·  ROCm 7.2  ·  bf16 supported ✅
Python 3.14.0
torch 2.12.0+rocm7.2   transformers 5.10.2   trl 1.4.0   peft 0.19.1   accelerate 1.13.0
fla (flash-linear-attention) 0.5.1  ✅ installed
flash_attn                          ❌ never installs on this machine
```

## Attention: what actually works here
...
- ⚠️ `lr_scheduler_kwargs` is **still commented out at `fine_tune.py:403`**, so `min_lr_rate` in
  a TOML is silently ignored. **Step 0 uncomments it** — the single sanctioned edit outside
  `training/patina/`
...
1. Uncomment `training/fine_tune.py:403` (`lr_scheduler_kwargs`). One line; the only edit
   outside `training/patina/`.
2. `training/patina/style_prompts.jsonl` — ~50 held-out prompts, with the allowed/banned
...
3. `training/patina/style_eval.py` — loads the prompt set, generates with the chat template at
...
4. Token-length histogram over arm A (`scripts/count_tokens.py`) to set `max_seq_length` at
...
Without this, no post-training number means anything.
...
attn_implementation = "eager"     # MANDATORY: sdpa NaNs non-deterministically, flex NaNs at step 1
...
bf16 = true                       # honoured since fine_tune.py:406
...
**⚠️ These criteria were revised after exp-01b exposed two flaws in them. Current form:**
...
🚨 **The circularity is documented, not hidden.** `p_pre1900` is also the function that filtered
...
rates Archangel prose at 100%. Read `samples.md` with your own eyes before declaring victory.
...
Stay in `training/patina/` and the specific files mentioned above — there's a ton of other
files and folders and you'll get lost. Sole sanctioned exception: uncommenting
`training/fine_tune.py:403`.
